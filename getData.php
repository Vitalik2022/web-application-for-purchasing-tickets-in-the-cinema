<?php	
	...
	$sql='select distinct S.id_film, F.* from sessions S, films F   
	      where S.id_film=F.id and S.dt between CURDATE() 
	...
	$sql='select S.*, F.film, F.genre, H.hallname, H.color, H.countplaces 
	      ...
	      where (S.id_film=F.id) and (S.id_hall=H.id) and 
	            (S.dt between CURDATE() and DATE_ADD(CURDATE(), INTERVAL 7 DAY))
	      order by S.DT, S.tbegin';
	...
	$sql='select T.* from tickets T  
	      where T.id_session in (select id from Sessions S
	                             where S.dt between CURDATE() and 
